...
(() => {
  ...
  let raf = false;
  addEventListener('scroll', () => { if (!raf) { raf = true; requestAnimationFrame(() => { progress(); raf = false; }); } }, {passive: true});
  progress();
  ...
  function updateInterface() {
    ...
    if (currentNote) {
      ...
      if ($('#reading-time')) $('#reading-time').textContent = t('readingTime',{n:Math.max(1,Math.ceil(han/350+words/220))});
