...
还有一个好玩的发现,其实我们是用谐振子和中心极限定理来描述溶液几乎一切行为.

## 本文主线

这篇文章的主线可以压缩成三步：1.从 **Anderson模型** 出发，先用 **Green函数** 把局域电子态和连续金属能带的耦合写成 **态密度（DOS）**；2.然后进入 **Newns–Anderson模型**，看吸附物能级怎样发生移动、展宽以及成键/反键分裂；3.最后进入 **Schmickler模型**，把电极电势、溶剂的**快变量和慢变量**以及**重组能**加入电子共享图像。

中间有一段关于谐振子、**中心极限定理**和 **Einstein 涨落理论**，很有趣, 解释为什么可以用简单的谐振子表示复杂溶剂。

![](images/paste-14.png){fig-align="center" width="723"}

**我们直接来看图片理解Newns–Anderson, 从图中b从右向左看起,孤立的气体分子有分子轨道2px,y,z,当他们靠近表面的时候就会与金属的d轨道相互作用.于是分子轨道与金属的d轨道波函数对称性匹配越好成键就越稳定,能量也就越低,2pz耦合的更好能量更低,形成了成键轨道和反键轨道,而2px,py轨道波函数匹配的不好形成了非键轨道(nb). 在最左图a中DOS(也就是金属如何安排它的电子在不同的离散能级,这里连续的画只是一个数学处理), a图红色的是Cu 3d轨道,也可以看到只有在费米能级以下填充了电子,填充到成键电子越多吸附相对越强,反之反键轨道填充越多吸附越弱.**
...
这里也有*Jülich* 的Jun Huang的视频 34:00介绍了一些格林函数用于anderson模型的推导细节.\[5\]
...
我们先用PZC**[PZC](Potential_of_Maximum_Entropy_PME_and_PZC_new.md)**和金属功函数直接的关系进入电化学的世界.
...
Trasatti筛选了一系列金属的WF和PZC数据，发现并不存在一个统一的规律，而是过渡金属和sp金属符合不同的规律。\[1\]

Trasatti进一步去分析过渡金属和sp金属为什么符合不同的规律。他发现金属表面对水分子极化起到了主导作用。过渡金属对界面水分子有很强的化学吸附作用，而sp金属仅是通过静电作用调整界面水分子的取向。把sp金属上水分子的取向极化与金属元素的电负性关联起来。\[1\]\[2\]

这里其实很有意思,那我们怎么具体用模型理解newns anderson的sp电子贡献吸附作用和sp金属贡献双电层贡献呢? 有空我就写一下.
...
也可以参考我们从静电学的角度推导的马库斯外重组能.**[Outer reorganization](electrostatics and Outer-sphere reorganization energy_new.md)**
