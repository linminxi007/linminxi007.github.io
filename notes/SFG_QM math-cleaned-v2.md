...
我们使用到的书有樱井纯的现代量子力学\[1\],阿金斯物理化学的分子对称性群论部分\[2\] ,Principles of Nonlinear Optical Spectroscopy: A Practical Approach or: Mukamel for Dummies Peter Hamm\[3\]
其实SO3等变神经网络中处理球张量的旋转和这里也是类似的.**[SO3神经网络](SO(3)-Equivariant Graph Neural Networks_new.md)**
...
上面不可交换性质直接决定了微观算符的代数结构，我们可以写出 SO(3) 群的核心对易关系\[1\]：$[J_i, J_j] = i\hbar \epsilon_{ijk} J_k$。也就是把“旋转操作是不可交换的”按照数学语言再写一遍。有了这个公式，只是方便我们底下的计算罢了。

## 二、角动量的“好基底”与代数结构

### 2.1 寻找“好基底”与总角动量算符 $J^2$
...
- 根据量子力学公理，相互对易的算符拥有共同的本征态。我们将 $J^2$ 和 $J_z$ 的共同本征态 $\vert{}j, m\rangle$ 选定为描述物理演化的完备“好基底”（Good Basis）。也就是说，它们俩相互对易，可以作为一组完备的量子数来描述一个分子的角动量状态。
...
- 到这里在球坐标下的坐标系和关系基本上导出完毕了.

### 2.4 Wigner-Eckart 定理的推导

Wigner-Eckart的定律推导我们先略过,可以直接看樱井纯的\[1\],极其优美.

我看完一次推导就忘记一次. 不过Wigner把群论引入量子力学一开始也让很多物理学家头痛,没人知道Wigner在做啥.所以我们搞不明白的话,也不算我们很笨.

## 三、不可约球张量与 Wigner-Eckart 定理

### 3.1 不可约球张量算符：把直角坐标扔进垃圾桶

- 我们在算极化率 $\beta$ 时，光电场是用实验室的 $x, y, z$ 直角坐标系描述的，比如写成偶极算符 $\mu_x, \mu_y, \mu_z$。

- 但是直角坐标系处理“旋转”极其笨拙。因为光子打进分子本质上是角动量的传递, 我们想必须换成跟角动量（$j, m$）说同一种语言的数学工具，这就是**不可约球张量算符** $T_q^{(k)}$。
...
### 3.2 Wigner-Eckart (WE) 定理：分解为几何+强度

- 量子力学里最让人头疼的就是算积分,因为你把波函数和态算再精巧,没有测量也就是加上\<\>框号做积分,你也没法在真实世界测到他们, 所以必须要project到现实世界. WE可以帮助计算跃迁矩阵元\[1\] $\langle j_f, m_f \vert T_q^{(k)} \vert j_i, m_i \rangle$。如果你要手算，每次分子转一下姿态、你换一个偏振光，都要重新算一遍三维积分。
...
- **偷懒**：在宏观非线性光学极化率计算中，如果没有 WE 定理，面对 $\beta_{zxy}$ 展开出的多条积分路径将是计算量的地狱。
...
因为关注的是非线性光学 **SFG / SHS** 的量子力学基础,Peter Hamm 与 Mukamel 的含时微扰密度矩阵理论通过非线性响应函数的推导，将外部宏观光场与分子内部的微观偶极跃迁矩阵元建立了严格的解析联系。\[3\]\[4\]

### 4.1 第一步：Liouville 空间内的嵌套对易子展开

这一步很简单你只要看他的书,从二态系统的密度矩阵到含时微扰你就可以得到下面的方程.

二阶密度矩阵在**相互作用绘景（Interaction Picture）**下的含时演化受制于 **Liouville-von Neumann 方程**的双重积分：
...
假设系统初始完全处于基态 $\rho^{(0)} = \vert{}a\rangle\langle a\vert{}$。将内层对易子展开，再展开外层对易子，会产生 4 项代数式：
...
为了展示和频产生（SFG）中连续吸收两个光子的一条贡献路径，我们可以在双侧费曼图中锁定纯 Ket 侧激发路径（即状态 $\vert{}a\rangle \xrightarrow{\omega_1} \vert{}b\rangle \xrightarrow{\omega_2} \vert{}c\rangle$）。这个过程等于于两个相互作用算符都作用在密度矩阵左侧的第一项：$\tilde{V}(t_2)\tilde{V}(t_1)\rho^{(0)}$。完整的二阶响应还需要把其他允许的 Liouville 路径和时间排序一起加起来^\[3\]\[4\]^。对于SFG,系统吸收两个光子，状态演化为 $\vert{}c\rangle\langle a\vert{}$。为了完成 SFG 发光过程，系统必须辐射出一个频率为 $\omega_1 + \omega_2$ 的光子，并从最高激发态 $\vert{}c\rangle$ 跃迁回基态 $\vert{}a\rangle$。

所以提取到达末态 $\vert{}c\rangle$ 的非对角元 $\rho_{ca}^{(2)}(t) = \langle c \vert{} \rho^{(2)}(t) \vert{} a \rangle$：
...
我们现在就可以利用Wigner–Eckart theorem,群论,对称性,奇函数偶函数是不是为0,等等你能想到的一切方法偷懒,试试看$\beta_{ijk}$有没有项是等于0.
...
$C_2(z)$这一点阿金斯的物理化学说的很清楚$^{[6]}$,我们把上面的操作总结为
...
这就是阿特金斯查 character table 的真正用法。
...
回到我们的问题,一个分子整体的$(\beta_{ijk})$ 张量分量能不能存在？ 公式(23)里面：
...
假设初态最终又回到同一个 (A1) 基态，那么三个偶极算符连乘必须整体是全对称：
...
道理很简单因为如果整个被积函数在某个对称操作下变号,这个积分就会出现**I=-I**,这样的情况那I就**等于0**了. 所以我们可以直接用：
...
\beta_{zxx} = 
\mu_z\mu_x\mu_x = A_1\otimes B_1\otimes B_1


$$

其中 $B_1\otimes B_1=A_1
$
...
我们还可以使用奇偶宇称的方法,**偶×奇=奇, 偶×偶=偶**,来判断是不是为0.

一般基态都是对称的偶函数,中间的偶极矩算符明显是奇函数,然后就看最左边的态是什么了,其实就是奇函数的对称积分为0,偶函数不为0的道理.
...
### 6.1 第一步：振动共振分母直接变成 ($\omega_q-\omega_{\rm IR}$)
...
### 6.2 第二步：右边单独的偶极矩阵元变成 ($\partial\mu/\partial Q_q$)
...
### 6.4 第四步：$\langle0|\alpha_{ij}|1_q\rangle$ 再变成 $\partial\alpha_{ij}/\partial Q_q$
...
$$
\mu'_{k,q} \equiv \left( \frac{\partial\mu_k}{\partial Q_q} \right)_0,
\qquad
\alpha'_{ij,q} \equiv \left( \frac{\partial\alpha_{ij}}{\partial Q_q} \right)_0.
\tag{89}
$$
...
这就是在非共振电子响应、Born–Oppenheimer 与 harmonic/Placzek 近似下常见的形式.

## 七、DFT 中的偶极矩、极化率与有限差分

至于没有为0的二阶极化率分量,我们就用WE硬算,或者用我们刚刚得到的关系偶极变化率乘以极化率变化率,我们往往用我的老本行**DFT**去算,偶极矩就在分子那三个标准的振动模式的方向上,比如比平衡位置多摆动一点和少摆动一点计算偶极的变化,然后做差分.咱们主要看看变化.然后计算极化率的变化率也是推拉一下,然后加个两个方向的X,Y,Z正负电场看看能量变化.
...
然后现在我们把分子的坐标系project到lab的坐标系,就是一个矩阵变化(欧拉角ZYZ顺规)比我的机器人自瞄项目简单多了.**[RoboMaster](robomaster_target_new.md)**
